...
// The emailed code is stored only as a hash, keyed by a hash of the email, and
...
export async function createParticipant(input) {
  ...
  const p = {
    ...
    // Indexed lower-case copy so passwordless sign-in can look a participant up
    // by the email they type without scanning the collection.
    email_normalized: String(input.email || '').trim().toLowerCase() || null,
