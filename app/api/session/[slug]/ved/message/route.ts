...
// One turn of the Execution Doctrine diagnostic.
export const POST = handler(async (req: Request, { params }: Ctx) => {
  ...
  if (!session.consent_given) await repo.updateSession(session.id, { consent_given: true });

  const { reply, messageCount } = await agentTurn({ participant: p, session, userMessage: msg, mode: "ved" });
  return ok({
    reply,
