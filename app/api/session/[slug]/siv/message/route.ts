...
// One turn of the SIV AI Project Selector examination.
export const POST = handler(async (req: Request, { params }: Ctx) => {
  ...
  if (!session.selected_depth) return fail("Please choose a depth to begin the examination.", 400);

  const { reply, messageCount } = await agentTurn({ participant: p, session, userMessage: msg, mode: "siv" });
  return ok({
    reply,
