...
const mount = document.querySelector("[data-signal-spacetime-logo]");

if (mount) {
  const fallback = mount.querySelector(".hero-logo-fallback");
  ...
  try {
    ...
    fallback?.classList.add("is-rendered");

    const group = new THREE.Group();
    group.rotation.x = 0.3;
    scene.add(group);

    scene.add(new THREE.HemisphereLight(0xffffff, 0x555565, 0.6));
    ...
    camera.position.copy(cameraDirection.multiplyScalar(5.5).add(target));
    ...
    for (let i = 0; i < 60; i += 1) {
      ...
      bar.position.set(Math.cos(angle) * 1.3, 1, Math.sin(angle) * 1.3);
    ...
    ringRadii.forEach((radius, index) => {
      ...
      const ring = new THREE.Mesh(
        new THREE.TorusGeometry(radius, index < 2 ? 0.008 : 0.006, 6, 144),
        material
      );
    ...
    class WellSpokeCurve extends THREE.Curve {
      ...
      getPoint(u, targetPoint = new THREE.Vector3()) {
        const radius = THREE.MathUtils.lerp(0.52, 2.1, u);
        return targetPoint.set(
    ...
    for (let i = 0; i < 16; i += 1) {
      ...
      grid.add(
        new THREE.Mesh(
          new THREE.TubeGeometry(new WellSpokeCurve(angle), 52, 0.0055, 5, false),
          spokeMaterial
        )
      );
    }
    ...
    const renderFrame = (t) => {
      bars.forEach((bar) => {
        ...
        const wave = Math.pow(0.5 + 0.5 * Math.sin(ph + t * 1.5), 1.6);
        bar.scale.y =
          0.14 +
          (hot ? 0.5 : 0.34) * wave +
      ...
      signalMaterials.forEach((material, k) => {
        material.emissiveIntensity =
          0.5 + 0.4 * (0.5 + 0.5 * Math.sin(t * 1.5 + k * 1.05));
      });
      ...
      gridRingMaterials.forEach((material, k) => {
        material.emissiveIntensity =
          0.35 +
          0.8 *
            Math.pow(0.5 + 0.5 * Math.sin(t * 1.6 - k * 0.85), 2.2);
      });
    ...
    window.addEventListener("pageshow", (event) => {
      if (event.persisted && !reduceMotion.matches) {
        animationFrame = requestAnimationFrame(animate);
      }
    });
  } catch (error) {
    mount.classList.add("hero-logo--fallback");
